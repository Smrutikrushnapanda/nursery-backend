...
export class PosService {
  ...
  async quickOrder(dto: QuickOrderDto, organizationId: string) {
    ...
    return {
      ...
      taxPercentage: Number(result.summary?.taxPercentage ?? result.order.taxPercentage ?? 0),
      taxAmount: Number(result.summary?.taxAmount ?? result.order.taxAmount ?? 0),
      taxBreakdown: result.summary?.taxBreakdown ?? [],
